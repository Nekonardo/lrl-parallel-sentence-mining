...
# Patch torch.compile to a no-op to avoid triton version incompatibility with ModernBERT.
# ModernBERT applies @torch.compile at class-definition time, which triggers a broken triton import.
...
def to_labse_sentence_embeddings(path, sentence_list, start_i=0):
    ...
    for i in tqdm(range(start_i + 1, len(sentence_list))):
        ...
        split_sentence = sentence.split('\t')
        assert len(split_sentence) == 2, f'The line contains too many fields: {len(split_sentence)}'
        str_embedding = str_embedding = get_labse_embeddings(split_sentence[1],
                                                             labse_model)  # get_embedding(split_sentence[1], xlmr_embeddings)
        if str_embedding:  # Not None
            embedding_list.append(f'{split_sentence[0]} {" ".join(str_embedding)}')
        if i % 10000 == 0:
            with open(path, 'a', encoding='utf8') as out_text:
...
# # def to_laser2_embeddings(path, sentence_list, start_i=0):
#     encoder = LaserEncoderPipeline(laser='laser2')
#     embedding_size = 1024
#     print(f'Loading Sentence-Transformer model:laser2')
...
#         split_sentence = sentence.split('\t')
#         assert len(split_sentence) == 2, f'The line contains too many fields: {len(split_sentence)}'
#         # str_embedding = get_labse_embeddings(split_sentence[1], st_model)  # Convert format
...
#         split_sentence = sentence.split('\t')
#         assert len(split_sentence) == 2, f'The line contains too many fields: {len(split_sentence)}'
#         # str_embedding = get_labse_embeddings(split_sentence[1],
#         #                                                      st_model)  # get_embedding(split_sentence[1], xlmr_embeddings)
#         str_embedding = encoder.encode_sentences(split_sentence[1])
#         if str_embedding:  # Not None
#             embedding_list.append(f'{split_sentence[0]} {" ".join(str_embedding)}')
#         if i % 10000 == 0:
...
def to_laser2_embeddings(path, sentence_list, start_i=0):
    ...
    if start_i == 0:
        ...
        split_sentence = sentence.split('\t')
        assert len(split_sentence) == 2, f'The line contains too many fields: {len(split_sentence)}'
        emb_matrix = encoder.encode_sentences([split_sentence[1]])
        str_embedding = [f"{v:.6f}" for v in emb_matrix[0]]
...
def to_sentence_transformer_embeddings(path, sentence_list, model_name, start_i=0):
    model_map = {
...
def main():
    ...
    if model_name in ['xlmr', 'glot500', 'pretrained', 'mmbert']:
        to_xlmr_sentence_embeddings(args.output_file, split_file, model_name, start_i=0)
    elif model_name in ['labse', 'mpnet', 'multi-qa-mpnet']:
        # to_labse_sentence_embeddings(args.output_file, split_file, start_i=0)
        to_sentence_transformer_embeddings(args.output_file, split_file, model_name, start_i=0)
    elif model_name in ['laser2']:
        to_laser2_embeddings(args.output_file, split_file, start_i=0)
